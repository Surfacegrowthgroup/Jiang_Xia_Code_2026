function [ress,HH]=MCMC_BDHFFGN_TC(L,time,cc,th0,pr,THETA1)
% This is the main function "main" for simulating the distribution of the height of the long-range time correlation BD model under the method of large deviation. 
...
for c=1:a(2)
    pr1=pr(c);
    R_Mx=rand(L,N0,time);
    H_Mx=BDHFFGN(L,time,th0,R_Mx);
    for p=1:cc
        R_My=CH_GE(R_Mx,pr1,L,N0,time);
        H_My=BDHFFGN(L,time,th0,R_My);
